#!/usr/bin/env python3
"""
Test script to verify that the app is using the updated calc_engine with fixed zone matrix.
"""
...
def test_app_zone_matrix():
    """Test that the app is using the updated calc_engine."""
    
    print("Testing app zone matrix loading...")
    ...
    # Initialize calculator with the same template path the app uses
    try:
    ...
    # Test a few zone lookups that should now work better
    test_cases = [
...
if __name__ == "__main__":
    success = test_app_zone_matrix()
    if success:
        print("\n🎉 App zone matrix test completed successfully!")
        print("The app should now be using the fixed zone matrix.")
    else:
        print("\n❌ App zone matrix test failed!")
        sys.exit(1) 